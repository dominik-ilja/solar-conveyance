...
import {Spinner, ImageElement} from '../components'
import { fetchRocketDataById } from "../constants"
...
export default function LaunchCard ({ launchData, ...props }) {
  ...
  useEffect(() => {
    ...
    async function fetchDataForRocket() {
        const data = await fetchRocketDataById(launchData.rocket)
        setRocket(data)
    }
    fetchDataForRocket()
  }, [launchData.rocket])
  ...
  return (
    <div className="flex flex-col text-white bg-gray-900">
        <div className="flex-1">
          {rocket ? <ImageElement images={rocket.flickr_images} /> : <Spinner className="mt-4" />}
        </div>

        <div className="px-4 py-8">
          <div className="mb-4 text-center">
            <div className="text-2xl">{launchData.name}</div>
            {rocket ? <div className="text-gray-600">{rocket.name}</div> : <Spinner className="mt-4" />}
          </div>
